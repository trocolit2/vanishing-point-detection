...
BOOST_AUTO_TEST_CASE(drawOrthogonalVP_testeCase){
  ...
  for (unsigned int i = 0; i < points.size(); i++) {
    ...
    BOOST_CHECK_CLOSE(points[i].x, max_point.x, 2.0);
    BOOST_CHECK_CLOSE(points[i].y, max_point.y, 2.0);
  }
}
...
BOOST_AUTO_TEST_CASE(checkYUDDataset_testCase){
  ...
  points[0] = cv::Point2f(yud_gt[0][0]/yud_gt[0][2],yud_gt[0][1]/yud_gt[0][2]);
  points[1] = cv::Point2f(yud_gt[0][3]/yud_gt[0][5],yud_gt[0][4]/yud_gt[0][5]);
  points[2] = cv::Point2f(yud_gt[0][6]/yud_gt[0][8],yud_gt[0][7]/yud_gt[0][8]);
